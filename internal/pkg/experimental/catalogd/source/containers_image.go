...
type ContainersImageRegistry struct {
	...
	IoReaderWrapper   func(reader io.Reader) io.Reader
}
...
func (i *ContainersImageRegistry) unpackImage(ctx context.Context, unpackPath string, imageReference types.ImageReference, specIsCanonical bool, sourceContext *types.SystemContext) error {
	...
	for idx, layerInfo := range img.LayerInfos() {
		if err := func() error {
			layerReader, _, err := layoutSrc.GetBlob(ctx, layerInfo, none.NoCache)
			if err != nil {
				return fmt.Errorf("error getting blob for layer[%d]: %w", idx, err)
			}
			...
			if _, err = archive.Apply(ctx, unpackPath, decompressed, archive.WithFilter(applyLayerFilter(dirToUnpack))); err != nil {
				return fmt.Errorf("error applying layer[%d]: %w", idx, err)
			}
			l.Info("applied layer", "layer", idx)
			return nil
		}(); err != nil {
...
func applyLayer(ctx context.Context, destPath string, srcPath string, layer io.Reader) error {
